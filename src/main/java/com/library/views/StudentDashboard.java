...
public class StudentDashboard extends BaseDashboard {
    ...
    protected JPanel createSidebar() {
        ...
        // Create buttons
        borrowBooksButton = createMenuButton("Borrow Books");
        returnBooksButton = createMenuButton("Return Books");
        viewStatusButton = createMenuButton("View Status");
        requestNewBooksButton = createMenuButton("Request New Books");
        reissueBooksButton = createMenuButton("Reissue Books");
        notificationsButton = createMenuButton("Notifications");
        ...
        for (JButton button : new JButton[]{borrowBooksButton, returnBooksButton, 
                viewStatusButton, requestNewBooksButton, reissueBooksButton, 
                notificationsButton, logoutButton}) {
    ...
    private void handleButtonClick(JButton button) {
        if (button == borrowBooksButton) {
            showBorrowBooksPanel();
        } else if (button == returnBooksButton) {
            showReturnBooksPanel();
        } else if (button == viewStatusButton) {
            showViewStatusPanel();
        } else if (button == requestNewBooksButton) {
            showRequestNewBooksPanel();
        } else if (button == reissueBooksButton) {
            showReissueBooksPanel();
        } else if (button == notificationsButton) {
    ...
    private void showBorrowBooksPanel() {
        ...
        panel.add(createHeaderPanel("Borrow Books"), BorderLayout.NORTH);

        // Toolbar for search and filter + prominent borrow button
        JPanel toolbarPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        JTextField searchField = new JTextField(20);
        JButton searchButton = new JButton("Search");
        JComboBox<String> categoryFilter = new JComboBox<>(new String[]{"All Categories", "Fiction", "Non-Fiction", "Science", "History", "Biography"});
        ...
        borrowButton.addActionListener(e -> {
            ...
            if (selectedRow == -1) {
                JOptionPane.showMessageDialog(panel, "Please select a book to borrow.", "No Selection", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            if (available <= 0) {
                JOptionPane.showMessageDialog(panel, "No available copies for this book.", "Unavailable", JOptionPane.WARNING_MESSAGE);
                return;
            }
            try (java.sql.Connection conn = com.library.utils.DatabaseConnection.getInstance().getConnection()) {
                ...
                try (java.sql.PreparedStatement countStmt = conn.prepareStatement("SELECT COUNT(*) FROM book_transactions WHERE user_id = ? AND status = 'BORROWED'")) {
                    countStmt.setInt(1, userId);
                    try (java.sql.ResultSet rs = countStmt.executeQuery()) {
                        if (rs.next() && rs.getInt(1) >= 3) {
                            JOptionPane.showMessageDialog(panel, "You have reached the maximum number of borrowed books (3).", "Limit Reached", JOptionPane.WARNING_MESSAGE);
                            return;
                        }
                    }
                }
                ...
                java.sql.Timestamp due = new java.sql.Timestamp(System.currentTimeMillis() + 14L * 24 * 60 * 60 * 1000); // 14 days
                ...
                try (java.sql.PreparedStatement updateStmt = conn.prepareStatement("UPDATE books SET available_quantity = available_quantity - 1 WHERE id = ?")) {
                    updateStmt.setInt(1, bookId);
                    updateStmt.executeUpdate();
                }
                JOptionPane.showMessageDialog(panel, "You have successfully borrowed '" + title + "'.", "Success", JOptionPane.INFORMATION_MESSAGE);
                loadBooks.run();
            } catch (java.sql.SQLException ex) {
                JOptionPane.showMessageDialog(panel, "Error borrowing book: " + ex.getMessage(), "Database Error", JOptionPane.ERROR_MESSAGE);
            }
        });
    ...
    private void showReturnBooksPanel() {
        JPanel panel = new JPanel(new BorderLayout());
        panel.add(createHeaderPanel("Return Books"), BorderLayout.NORTH);
        ...
        Runnable loadBorrowed = () -> {
            ...
            try (java.sql.Connection conn = com.library.utils.DatabaseConnection.getInstance().getConnection()) {
                ...
                JOptionPane.showMessageDialog(panel, "Error loading borrowed books: " + ex.getMessage(), "Database Error", JOptionPane.ERROR_MESSAGE);
            }
        };
        loadBorrowed.run();
        ...
        returnButton.addActionListener(e -> {
            ...
            if (selectedRow == -1) {
                JOptionPane.showMessageDialog(panel, "Please select a book to return.", "No Selection", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            try (java.sql.Connection conn = com.library.utils.DatabaseConnection.getInstance().getConnection()) {
                ...
                try (java.sql.PreparedStatement pstmt = conn.prepareStatement("UPDATE book_transactions SET status = 'RETURNED', return_date = ? WHERE id = ?")) {
                    pstmt.setTimestamp(1, new java.sql.Timestamp(System.currentTimeMillis()));
                    pstmt.setInt(2, transactionId);
                    pstmt.executeUpdate();
                }
                // Increment available_quantity
                try (java.sql.PreparedStatement getBookIdStmt = conn.prepareStatement("SELECT book_id FROM book_transactions WHERE id = ?")) {
                    getBookIdStmt.setInt(1, transactionId);
                    try (java.sql.ResultSet rs = getBookIdStmt.executeQuery()) {
                        if (rs.next()) {
                            int bookId = rs.getInt("book_id");
                            try (java.sql.PreparedStatement updateBookStmt = conn.prepareStatement("UPDATE books SET available_quantity = available_quantity + 1 WHERE id = ?")) {
                ...
                JOptionPane.showMessageDialog(panel, "Error returning book: " + ex.getMessage(), "Database Error", JOptionPane.ERROR_MESSAGE);
            }
        });
        showContent(panel);
    }
    ...
    private void showViewStatusPanel() {
        JPanel panel = new JPanel(new BorderLayout());
        panel.add(createHeaderPanel("View Status"), BorderLayout.NORTH);
    ...
    private void showRequestNewBooksPanel() {
        JPanel panel = new JPanel(new BorderLayout());
        panel.add(createHeaderPanel("Request New Books"), BorderLayout.NORTH);
    ...
    private void showReissueBooksPanel() {
        JPanel panel = new JPanel(new BorderLayout());
        panel.add(createHeaderPanel("Reissue Books"), BorderLayout.NORTH);
        ...
        reissueButton.addActionListener(e -> {
            int selectedRow = borrowedTable.getSelectedRow();
            if (selectedRow == -1) {
                JOptionPane.showMessageDialog(panel, "Please select a book to reissue.", "No Selection", JOptionPane.WARNING_MESSAGE);
                return;
            }
            int transactionId = (int) tableModel.getValueAt(selectedRow, 0);
            String reissued = (String) tableModel.getValueAt(selectedRow, 5);
            if ("Yes".equals(reissued)) {
                JOptionPane.showMessageDialog(panel, "This book has already been reissued once.", "Limit Reached", JOptionPane.WARNING_MESSAGE);
                return;
            }
            try (java.sql.Connection conn = com.library.utils.DatabaseConnection.getInstance().getConnection()) {
                // Extend due date by 14 days and set transaction_type to REISSUE
                try (java.sql.PreparedStatement pstmt = conn.prepareStatement("UPDATE book_transactions SET due_date = DATE_ADD(due_date, INTERVAL 14 DAY), transaction_type = 'REISSUE' WHERE id = ?")) {
                    pstmt.setInt(1, transactionId);
                    pstmt.executeUpdate();
                }
                JOptionPane.showMessageDialog(panel, "Book reissued successfully! Due date extended by 14 days.", "Success", JOptionPane.INFORMATION_MESSAGE);
                loadBorrowed.run();
            } catch (java.sql.SQLException ex) {
                JOptionPane.showMessageDialog(panel, "Error reissuing book: " + ex.getMessage(), "Database Error", JOptionPane.ERROR_MESSAGE);
            }
        });
        showContent(panel);
    }
    ...
    private void showNotificationsPanel() {
        JPanel panel = new JPanel(new BorderLayout());
        panel.add(createHeaderPanel("Notifications"), BorderLayout.NORTH);
